...
void OLED_IO_Init(void)
{
  ...
  //GPIOx配置
  GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;	//OLED  PB12/13/14/15 端口配置
  GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP; 		 //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		   //IO口速度为50MHz
  GPIO_Init(GPIOB, &GPIO_InitStructure);					       //根据设定参数初始化GPIOA PA2
