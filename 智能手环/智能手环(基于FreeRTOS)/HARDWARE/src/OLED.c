...
void OLED_IO_Init(void)
{
  ...
  /*GPIOx端口配置*/
  //PB5、7、13、15
  GPIO_InitStruc.GPIO_Pin   = (GPIO_Pin_3|GPIO_Pin_5|GPIO_Pin_7|GPIO_Pin_13);   //选择PB3、5、7、13
  ...
  GPIO_Init(GPIOB,&GPIO_InitStruc);                 //根据设定参数初始化PB3、5、7、15
	
	//PA15
	GPIO_InitStruc.GPIO_Pin = GPIO_Pin_15;            //选择PA15
	GPIO_Init(GPIOA,&GPIO_InitStruc);                 //根据设定参数初始化PA15
}
...
void OLED_ShowChar(u16 x,u8 y,u8 size,u8 ch)
{
	...
	if(size == 12)
  {
    ...
    //显示这一页的字符数据(1页=8行)
    for(i=0;i<6;i++)
		OLED_WR_Byte(CH12x8[c*6+i],OLED_DATA);
  }
  else if(size == 16)
  {
    /*注意：16的字符由2页组成*/
...
/*************************************************************************
*函数功能  ：OLED显示一个摄氏度单位(为了打印℃而做的函数)
*函数名    ：OLED_ShowT
*函数参数  ：u8 x  u8 y   u8 ch
*函数返回值：void
*描述      ：
***************************************************************************/
